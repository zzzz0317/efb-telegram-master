    ...
    def generate_message_template(self, msg: EFBMsg, tg_chat, multi_slaves: bool) -> str:
        ...
        if msg.chat.chat_type == ChatType.User:
            ...
            if msg.chat != msg.author:
                ...
                loggertxt = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + ' - ' + json.loads(str(self.bot.get_me())).get['username'] + ' sent a message to ' + ETMChat(chat=msg.chat, db=self.db).display_name
